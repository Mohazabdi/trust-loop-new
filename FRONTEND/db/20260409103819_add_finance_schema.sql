...
v_is_active := public.parse_boolean(p_is_active,TRUE);
    SELECT public.check_field_existance(p_created_by, 'finance', 'administrators') INTO v_result;
...
v_acc_status := p_acc_status::finance.acc_status;
    SELECT public.check_field_existance(p_owner_entity_id, 'finance', 'entities') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
            RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
v_max_transfer_amount := p_max_transfer_amount;
    SELECT public.check_field_existance(p_created_by, 'finance', 'entities') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
            RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
BEGIN
    ...
    SELECT public.check_field_existance(p_entity_id, 'finance', 'entities') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
            RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
    ...
    SELECT public.check_field_existance(p_granted_by, 'finance', 'entities') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
            RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
BEGIN
    ...
    ELSE
        ...
        SELECT public.check_field_existance(p_from_entity_id,'finance','entities') INTO v_result;
        IF NOT (v_result->>'success')::boolean THEN
                    RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
v_notification_type:=p_notification_type::finance.notification_type;
    ...
    SELECT public.check_field_existance(p_to_entity_id,'finance','entities') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
                RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
v_group_description := COALESCE(TRIM(p_group_description), '');
    ...
    SELECT public.check_field_existance(p_created_by, 'finance', 'members') INTO v_result;
    IF NOT (v_result->>'success')::boolean THEN
                RAISE EXCEPTION '%', v_result->>'message'
            USING ERRCODE = COALESCE(v_result->>'error_code', 'P0001'),
                  HINT = v_result->>'detail';
...
--3.initiator_id
SELECT public.check_field_existance(p_initiator_id,'finance','entities') INTO v_result;
IF NOT(v_result->>'success')::boolean THEN
RAISE EXCEPTION '%',v_result ->>'message'
USING ERRCODE=COALESCE(v_result->>'error_code','P0001'),
HINT =COALESCE(v_result->>'detail', 'No additional hint available');
...
SELECT public.check_field_existance(p_created_by_id,'finance','administrators') INTO v_result;
IF NOT (v_result->>'success')::boolean THEN
RAISE EXCEPTION '%',v_result->>'message'
USING ERRCODE =COALESCE(v_result->>'error_code','P0001'),
HINT =v_result ->>'detail';
