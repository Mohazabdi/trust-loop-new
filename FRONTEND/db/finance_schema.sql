...
DECLARE
    v_wallet_record RECORD;
BEGIN
    SELECT w.id, w.wallet_name, w.wallet_number, w.wallet_type, w.wallet_status
    INTO v_wallet_record
    FROM finance.wallets w
    INNER JOIN finance.wallet_access wa 
        ON wa.wallet_id = w.id 
        AND wa.entity_id = p_entity_id 
        AND wa.access_level IN ('view', 'manage')  -- only wallets user can see
