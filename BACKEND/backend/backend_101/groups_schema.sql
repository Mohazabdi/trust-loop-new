...
ALTER TABLE groups.rotation_plan_members ADD CONSTRAINT rotation_plan_members_rotation_plan_id_fkey FOREIGN KEY (rotation_plan_id) REFERENCES groups.rotation_plan (id);
 ...
 ALTER TABLE groups.rotation_plan
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT now();

-- Create ENUM for rotation member activity status
CREATE TYPE groups.rotation_member_status AS ENUM ('active', 'inactive', 'deleted');

-- Add column to rotation_plan_members (default 'active')
ALTER TABLE groups.rotation_plan_members
ADD COLUMN IF NOT EXISTS rotation_member_status groups.rotation_member_status DEFAULT 'active';

-- Index for faster filtering
CREATE INDEX IF NOT EXISTS idx_rotation_plan_members_status
ON groups.rotation_plan_members (rotation_member_status)
WHERE rotation_member_status = 'active';

ALTER TYPE groups.rotation_collection_schedule_status ADD VALUE IF NOT EXISTS 'partial';
...
-- Optionally add a trigger to update updated_at on row change
CREATE OR REPLACE FUNCTION groups.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_rotation_penalty_applied_updated_at ON groups.rotation_penalty_applied;
CREATE TRIGGER trg_rotation_penalty_applied_updated_at
    BEFORE UPDATE ON groups.rotation_penalty_applied
    FOR EACH ROW
    EXECUTE FUNCTION groups.update_updated_at_column();